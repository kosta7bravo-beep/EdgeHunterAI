...
def diagnostic_markets(
    ...
    for match in matches:
        ...
        for bookmaker_name, market_list in (
            bookmakers.items()
        ):
    ...
    return (
        f"🏦 Букмекеры:\n"
        f"{', '.join(sorted(bookmaker_names))}\n\n"

        f"📊 Поддерживаемые типы:\n"
        f"{', '.join(parsed_types)}\n\n"

        f"🎯 Разобранные рынки:\n"
        f"{', '.join(parsed_markets)}\n\n"

        f"📦 Позиций коэффициентов: "
        f"<b>{len(all_markets)}</b>\n"
